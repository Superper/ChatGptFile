...
void TcpServer::MakeSocket(const std::string &topic_id, const socket_cb &cb, const uint16_t &band_port,
                           const std::string &ip, const uint16_t &local_band_prot = 10000)
{
    topic_id_ = topic_id;
    socket_msg_cb_ = cb;
    ...
    threads_ = std::make_unique<std::thread>([=]() {
        ...
        auto msg = std::string("Tcp Server Listening on ") + ip + std::string(":") + std::to_string(band_port);
        ...
        while ((client_fd_ = accept(server_fd_, (struct sockaddr *)&address, (socklen_t *)&addrlen)))
        {
            ...
            else
            {
                ...
                auto msg = std::string("TCP Client: ") + client_ip + ":" + std::to_string(client_port) + " connected";
                socket_state_cb_(msg);
                ...
                tcp_clients_[client_fd_] = socket_info;
