...
void SocketConnectionManager::TcpSendData(const std::string &addr, const char *data, uint32_t size)
{
    if (size > 0)
    {
        SocketBase::TcpSendData(addr, data, size);
    }
}
SocketConnectionManager::~SocketConnectionManager()
{
