...
class SocketConnectionManager
{
  public:
    ...
    bool AddConnection(const std::string topic_name, SocketType soket_type, uint16_t port,
                       std::string inet_addr = "127.0.0.1", uint16_t local_port = 10000);
    ...
    void SetSocketStateCallback(socket_state cb);
    static void TcpSendData(const std::string &addr, const char *data, uint32_t size);
