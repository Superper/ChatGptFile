...
// 函数用于在大端和小端之间转换 64 位数据
inline uint64_t swap_endian64(uint64_t value)
{
...
inline uint64_t GetPkgTime()
{
    ...
    // 输出秒数和纳秒数
    // std::cout << "Seconds since 1970: " << seconds.count() << std::endl;
    // std::cout << "Nanoseconds since the last second: " << nanoseconds.count() << std::endl;

    // 获取高 4 字节（秒偏移量）和低 4 字节（纳秒偏移值）
    uint32_t high = static_cast<uint32_t>(seconds.count());
    uint32_t low = static_cast<uint32_t>(nanoseconds.count());
    uint64_t time;
    memcpy(&time, &high, sizeof(high));
    memcpy(reinterpret_cast<char *>(&time) + sizeof(high), &low, sizeof(low));
    return time;
}
...
const uint16_t MYID = 0xAA01;
static uint16_t SRCID;
...
{
    PkgHeader head;
    uint8_t img_name[256] = {0};    // 由回放软件填写系统全局命名名称
    uint16_t totalPackageNum;       // 图像数据的总包数，取值范围：[]
    uint32_t subPackageSeq; // 子包序号，取值范围：1~0xFFFFFF
    ...
    // uint8_t img_msg[dataLength];          // +N 图像数据
};
...
struct OtherSARMsg1
{
    ...
    uint32_t subPacketNumber ; // 子包序号：取值范围；1~0xFFFFFF
...
struct ImageDetectionResult
{
    struct TargetMsg
    {
        uint8_t image_storage_path[256] = {'i','m','g','p','a','s'}; // 目标图像存储路径，存入FTP，各家约定默认存储路径，该字段填写图像名称，如1.BMP
        uint64_t tar_position_msg; // 目标位置信息
        ...
        uint32_t target_image_center_pixel_coordinate
            [2]={1,2}; // 目标图像中心点像素坐标，图像坐标系为左上角为原点，x轴向右，y轴向下：a[0]：中心点x坐标，a[1]：中心点y坐标，单位：像素，LSB
                 // = 0.01
    ...
    PkgHeader header;          // 包头
    uint8_t cmd_id[20] = {'c', 'o', 'm', 'm', 'a', 'n', 'd', '1', '2', '3'} ;                          // 命令编号
    uint64_t recv_timestamp;   // 接收时间戳
    uint64_t finish_timestamp; // 处理完成时间戳
    uint8_t
        is_target; // 图像目标有无标识  0:无目标（只需填写“目标图像组成”“目标图像存储路径”填写“图像全局名称） 1:有目标
    uint8_t
        target_composition; // 目标图像组成，bit0：表示可见光；0-无，1-有；bit1：表示红外；0-无，1-有；bit2：表示SAR；0-无，1-有；其他预留
    uint8_t target_id[24] = {'t', 'a', 'r', 'g', 'e', 't', 'i', 'd'}; // 目标编号，参见目标编号命名规则
    uint8_t
    ...
    uint16_t tar_xh;          // 目标型号
    uint8_t tar_ser_air_type; // 目标海空类别
    uint8_t country_local[8] = {'c', 'o', 'n', 't', 'r', 'y'};                         // 国家和地区
    uint8_t dw;               // 敌我属性
...
struct TargDetectionImgMsg1
{
    ...
    uint32_t sub_package_index; // 子包序号，取值范围；1~0xFFFFFF。
...
{
    PkgHeader header;
    char CmdNo[20] = {0};      // 回复信息
    uint8_t control_type;      // 0x01：广域成像WZ目标智能检测；
    uint8_t operator_type = 0; // 1: 启动， 2：停止
    PkgTail tail;
};
struct ControlMsgReply
{
    PkgHeader header;
    char CmdNo[20] = {0};  // 回复信息
    uint8_t control_type;  // 0: 无效， 1：有效
    uint8_t operator_type; // 1: 启动， 2：停止
    uint8_t reply_type;    // 0: 无效， 1：成功， 2：失败
    PkgTail tail;
};
...
struct IntelligentProcessingResult
{
    PkgHeader header;
    char cmd_no[20] = {0};
    uint8_t target_no[16];
    uint8_t target_src;
    uint8_t mark;
    PkgTail tail;
};

struct QueHealthState
{

    PkgHeader header;
    uint8_t cmd_no[20];
    uint8_t sys_type = 1;       // 分系统类型  广域
    uint8_t hardware_count = 4; // 硬件数量
    uint8_t software_count = 2; // 软件数量
    uint8_t back;

    // 重复硬件数量次数
    uint8_t hw1_name[32] = {'h', '1'}; // 硬件名称
    uint8_t hw1_state = 1;             // 状态
    uint8_t hw1_back[3] = {0};

    uint8_t hw2_name[32] = {'h', '2'}; // 硬件名称
    uint8_t hw2_state = 2;             // 状态
    uint8_t hw2_back[3] = {0};

    uint8_t hw3_name[32] = {'h', '3'}; // 硬件名称
    uint8_t hw3_state = 3;             // 状态
    uint8_t hw3_back[3] = {0};

    uint8_t hw4_name[32] = {'h', '4'}; // 硬件名称
    uint8_t hw4_state = 4;             // 状态
    uint8_t hw4_back[3] = {0};

    // 重复软件数量次数
    uint8_t sw1_name[32] = {'s', '1'};
    uint8_t sw1_state = 1; // 1
    uint8_t sw1_back[3];

    uint8_t sw2_name[32] = {'s', '2'};
    uint8_t sw2_state = 2; // 1
    uint8_t sw2_back[3];
